...
import { DebugTilesRenderer as TilesRenderer } from "3d-tiles-renderer/index.js";

let camera_g, scene_g, renderer_g, tiles_renderer_g;
...
function render() {
    camera_g.updateMatrixWorld();
    tiles_renderer_g.update();
...
function init() {
    ...
    tiles_renderer_g = new TilesRenderer(get_absolute_url("tileset.json"));
    tiles_renderer_g.setCamera(camera_g);
    tiles_renderer_g.setResolutionFromRenderer(camera_g, renderer_g);
    tiles_renderer_g.displayBoxBounds = true;  // debug
    tiles_renderer_g.errorTarget = 1.0;
    scene_g.add(tiles_renderer_g.group);
